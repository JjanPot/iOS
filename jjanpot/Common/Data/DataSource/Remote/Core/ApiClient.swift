...
public class ApiClient<R: Router> {
    ...
    private func handleResponse<T: Decodable>(
        ...
        // 요청 에러 처리
        if let error = handleRequestError(result.error) {
        ...
        if 200..<300 ~= response.statusCode {
            return parseSuccessResponse(data: result.data)
        } else {
            return parseErrorResponse(statusCode: response.statusCode, data: result.data)
        }
    }

    private func handleRequestError(_ error: Error?) -> NetworkError? {
    ...
    private func parseSuccessResponse<T: Decodable>(data: Data?) -> Result<T, NetworkError> {
    ...
    private func parseErrorResponse<T: Decodable>(
        statusCode: Int,
        data: Data?
    ) -> Result<T, NetworkError> {
        ...
        if let errorBody = try? decoder.decode(ErrorResponseBody.self, from: data) {
            return .failure(.serverFailed(code: errorBody.status ?? statusCode,
